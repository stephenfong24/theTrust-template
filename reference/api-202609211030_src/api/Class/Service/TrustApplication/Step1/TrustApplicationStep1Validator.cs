...
namespace API_CPX.Class.Service.TrustApplication.Step1
{
    public class TrustApplicationStep1Validator
    {
        ...
        public void Validate(TrustApplicationStep1Request request)
        {
            ...
            if (!Validation.isValidIC(request.IdentityNo))
                throw new BusinessException("Invalid identification number. Please enter letters and numbers only, without spaces or special characters.", Code);
            ...
            if (!Validation.IsValidEmail(request.Email))
                throw new BusinessException("Email is not in valid format.", Code);
            ...
            if (string.IsNullOrWhiteSpace(request.EmployerName))
                throw new BusinessException("Employee/Name of company is required.", Code);

            if (string.IsNullOrWhiteSpace(request.NatureOfBusiness))
                throw new BusinessException("Nature of business is required.", Code);

            if (string.IsNullOrWhiteSpace(request.Occupation))
                throw new BusinessException("Occupation is required.", Code);
