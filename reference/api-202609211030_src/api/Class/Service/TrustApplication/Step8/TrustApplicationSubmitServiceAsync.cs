...
namespace API_CPX.Class.Service.TrustApplication.Step8
{
    public class TrustApplicationSubmitServiceAsync
    {
        ...
        public async Task<TrustApplicationStepResult> SubmitAsync(string merchantId, long userId, string roleCode, SubmitTrustApplicationRequest request)
        {
            ...
            {
                throw new BusinessException("Only Trust Agents are allowed to submit a new Trust Application.", Code);
            }
            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    await ValidateStep3Async(db, application.RowID);

                    await ValidateStep4Async(db, application.RowID);

                    await ValidateStep5Async(db, application.RowID);
                    ...
                    application.LastCompletedStep = 8;
                    ...
                    TrustApplicationStatusHelper.ChangeStatus(db, application, "PENDING_PAYMENT_APPROVAL", userId, "Trust Application submitted for payment approval.");
                    ...
                    TrustApplicationHistoryHelper.Add(
                        ...
                        "Trust Application " + application.TrustID.ToString("D4") + " was submitted for payment approval.",
                        userId,
                        "APPLICATION",
                        application.RowID,
                        "DRAFT",
        ...
        private async Task ValidateStep1Async(Sandbox_BasedEntities db, long trustApplicationId)
        {
            // =========================================================
            // Personal Detail
            // =========================================================
            ...
            {
                throw new BusinessException("Step 1 Personal Details are incomplete.", Code);
            }
            ...
            {
                throw new BusinessException("Step 1 Full Name is required.", Code);
            }

            if (string.IsNullOrWhiteSpace(personalDetail.IdentityType))
            {
                throw new BusinessException("Step 1 Identity Type is required.", Code);
            }

            if (string.IsNullOrWhiteSpace(personalDetail.IdentityNo))
            {
                throw new BusinessException("Step 1 Identity Number is required.", Code);
            }

            if (string.IsNullOrWhiteSpace(personalDetail.Nationality))
            {
                throw new BusinessException("Step 1 Nationality is required.", Code);
            }

            if (string.IsNullOrWhiteSpace(personalDetail.Email))
            {
                throw new BusinessException("Step 1 Email is required.", Code);
            }

            if (string.IsNullOrWhiteSpace(personalDetail.ContactNo))
            {
                throw new BusinessException("Step 1 Contact Number is required.", Code);
            }
            ...
            if (!hasSourceOfFund)
            {
                throw new BusinessException("Step 1 Source of Fund is required.", Code);
            }
        }
        ...
        private async Task ValidateStep2Async(Sandbox_BasedEntities db, long trustApplicationId)
        {
            ...
            if (trustAsset == null)
            {
                throw new BusinessException("Step 2 Trust Asset is incomplete.", Code);
            }
            ...
            {
                throw new BusinessException("Step 2 Trust Asset Amount must be greater than 0.", Code);
            }
            ...
            if (string.IsNullOrWhiteSpace(trustAsset.SettlorBankName))
            {
                throw new BusinessException("Step 2 Settlor Bank Name is required.", Code);
            }

            if (string.IsNullOrWhiteSpace(trustAsset.SettlorBankAccountHolder))
            {
                throw new BusinessException("Step 2 Settlor Bank Account Holder is required.", Code);
            }

            if (string.IsNullOrWhiteSpace(trustAsset.SettlorBankAccountNumber))
            {
                throw new BusinessException("Step 2 Settlor Bank Account Number is required.", Code);
            }
            ...
            if (string.IsNullOrWhiteSpace(trustAsset.GuaranteedReturnOption))
            {
                throw new BusinessException("Step 2 Guaranteed Return Option is required.", Code);
            }
            ...
            if (string.IsNullOrWhiteSpace(trustAsset.PaymentSource))
            {
                throw new BusinessException("Step 2 Payment Source is required.", Code);
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(trustAsset.JointAccountHolderName))
                {
                    throw new BusinessException("Step 2 Joint Account Holder Name is required.", Code);
                }
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(trustAsset.ThirdPartyName))
                {
                    throw new BusinessException("Step 2 Third Party Name is required.", Code);
                }

                if (string.IsNullOrWhiteSpace(trustAsset.ThirdPartyIdentityNo))
                {
                    throw new BusinessException("Step 2 Third Party Identity Number is required.", Code);
                }

                if (string.IsNullOrWhiteSpace(trustAsset.ThirdPartyRelationship))
                {
                    throw new BusinessException("Step 2 Third Party Relationship is required.", Code);
                }

                if (string.IsNullOrWhiteSpace(trustAsset.ThirdPartyBankName))
                {
                    throw new BusinessException("Step 2 Third Party Bank Name is required.", Code);
                }

                if (string.IsNullOrWhiteSpace(trustAsset.ThirdPartyBankAccountHolder))
                {
                    throw new BusinessException("Step 2 Third Party Bank Account Holder is required.", Code);
                }

                if (string.IsNullOrWhiteSpace(trustAsset.ThirdPartyBankAccountNumber))
                {
                    throw new BusinessException("Step 2 Third Party Bank Account Number is required.", Code);
                }
            }
        }
        ...
        private async Task ValidateStep3Async(Sandbox_BasedEntities db, long trustApplicationId)
        {
            var beneficiaries =
                await db.tbl_TrustApplication_Beneficiary
                    .Where(a => a.TrustApplicationID == trustApplicationId && a.IsActive)
                    .ToListAsync();
            ...
            {
                throw new BusinessException("Step 3 requires at least one active beneficiary.", Code);
            }
            ...
            foreach (var beneficiary in beneficiaries)
            {
                ...
                {
                    throw new BusinessException("Step 3 contains a beneficiary without a Full Name.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.IdentityType))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without an Identity Type.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.IdentityNo))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without an Identity Number.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.Nationality))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without Nationality.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.RelationshipCode))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without a Relationship.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.AddressLine1))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without an Address.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.Postcode))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without a Postcode.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.City))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without a City.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.State))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without a State.", Code);
                }

                if (string.IsNullOrWhiteSpace(beneficiary.Country))
                {
                    throw new BusinessException("Step 3 contains a beneficiary without a Country.", Code);
                }
            }
        }
        ...
        private async Task ValidateStep4Async(Sandbox_BasedEntities db, long trustApplicationId)
        {
            // =========================================================
            // Allocation Header
            // =========================================================
            ...
            if (allocation == null)
            {
                throw new BusinessException("Step 4 Beneficiary Allocation is incomplete.", Code);
            }
            ...
            if (allocation.AllocationType < 1 || allocation.AllocationType > 7)
            {
                throw new BusinessException("Step 4 contains an invalid Allocation Type.", Code);
            }
            ...
            var details =
                await db.tbl_TrustApplication_BeneficiaryAllocationDetail
                    .Where(a => a.AllocationID == allocation.RowID)
                    .ToListAsync();

            if (!details.Any())
            {
                throw new BusinessException("Step 4 Beneficiary Allocation Details are incomplete.", Code);
            }
            ...
            foreach (var detail in details)
            {
                ...
                {
                    throw new BusinessException("Step 4 contains an invalid beneficiary allocation.", Code);
                }

                if (!activeBeneficiaryIds.Contains(detail.BeneficiaryID.Value))
                {
                    throw new BusinessException("Step 4 references a beneficiary that is no longer active. Please review the Beneficiary Allocation.", Code);
                }
            }
            ...
            {
                case 1:
                    ValidateAllocationType1(details);
                    break;
                case 2:
                    ValidateAllocationType2(details);
                    break;
                case 3:
                    ValidateAllocationType3(details);
                    break;
                case 4:
                    ValidateAllocationType4(details);
                    break;
                case 5:
                    ValidateAllocationType5(details);
                    break;
                case 6:
                    ValidateAllocationType6(details);
                    break;
                case 7:
                    ValidateAllocationType7(details);
                    break;
            }
        }
        ...
        private async Task ValidateStep5Async(Sandbox_BasedEntities db, long trustApplicationId)
        {
            var execution =
            ...
            if (execution == null)
            {
                throw new BusinessException("Step 5 Execution of Trust Deed is incomplete.", Code);
            }
            ...
            if (string.IsNullOrWhiteSpace(execution.SigningMethod))
            {
                throw new BusinessException("Step 5 Signing Method is required.", Code);
            }
            ...
            if (string.IsNullOrWhiteSpace(execution.SpecialCircumstance))
            {
                throw new BusinessException("Step 5 Special Circumstance is required.", Code);
            }

            string specialCircumstance = execution.SpecialCircumstance.Trim().ToUpperInvariant();
            ...
            {
                if (string.IsNullOrWhiteSpace(execution.ReadOverBy))
                {
                    throw new BusinessException("Step 5 Read Over By is required for the selected special circumstance.", Code);
                }

                if (string.IsNullOrWhiteSpace(execution.ReadOverIdentityNo))
                {
                    throw new BusinessException("Step 5 Read Over Identity Number is required for the selected special circumstance.", Code);
                }

                if (string.IsNullOrWhiteSpace(execution.LanguageOrDialect))
                {
                    throw new BusinessException("Step 5 Language or Dialect is required for the selected special circumstance.", Code);
                }

                if (string.IsNullOrWhiteSpace(execution.RelationshipWithSettlor))
                {
                    throw new BusinessException("Step 5 Relationship With Settlor is required for the selected special circumstance.", Code);
                }

                if (string.Equals(execution.RelationshipWithSettlor, "OTHER", StringComparison.OrdinalIgnoreCase) && string.IsNullOrWhiteSpace(execution.OtherRelationshipWithSettlor))
                {
                    throw new BusinessException("Step 5 Other Relationship With Settlor is required.", Code);
                }
            }
        }
        ...
        private bool IsRole(string roleType, string expectedRole)
        {
            return string.Equals(roleType, expectedRole, StringComparison.OrdinalIgnoreCase);
        }
        ...
        private void ValidateOneHundredPercent(tbl_TrustApplication_BeneficiaryAllocationDetail detail, string errorMessage)
        {
            if (!detail.AllocationPercentage.HasValue || detail.AllocationPercentage.Value != 100m)
            {
        ...
        private void ValidateNoTrusteeCompany(List<tbl_TrustApplication_BeneficiaryAllocationDetail> details, string errorMessage)
        {
            foreach (var detail in details)
            {
                if (detail.IsTrusteeCompany || !detail.BeneficiaryID.HasValue)
                {
                    throw new BusinessException( errorMessage, Code);
                }
            }
        ...
        private void ValidatePercentageTotal(List<tbl_TrustApplication_BeneficiaryAllocationDetail> details, string errorMessage)
        {
            foreach (var detail in details)
            {
                ...
                if (detail.AllocationPercentage.Value <= 0 || detail.AllocationPercentage.Value > 100)
                {
                    throw new BusinessException(errorMessage, Code);
                }
            }

            decimal total = details.Sum(a => a.AllocationPercentage ?? 0m);
