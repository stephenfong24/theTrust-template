...
namespace API_CPX.Class.Service.TrustApplication.Step3
{
    public class TrustApplicationStep3ServiceAsync
    {
        ...
        private async Task SaveCaretakerDistributionAsync(
            Sandbox_BasedEntities db,
            long trustApplicationId,
            long userId,
            TrustApplicationCaretakerDistributionRequest request)
        {
            var existingCaretakers =
                await db.tbl_TrustApplication_Caretaker
                    .Where(x => x.TrustApplicationID == trustApplicationId)
                    .ToListAsync();
            ...
            {
                var existingSubstitute =
                    existingCaretakers.FirstOrDefault(x =>
        ...
        private void SaveCaretaker(
            Sandbox_BasedEntities db,
            List<tbl_TrustApplication_Caretaker> existingCaretakers,
            long trustApplicationId,
            long userId,
            string caretakerType,
            TrustApplicationCaretakerRequest request)
        {
            var caretaker =
                existingCaretakers.FirstOrDefault(x =>
                    x.CaretakerType == caretakerType);
        ...
        private async Task SaveMinorDistributionAsync(
            Sandbox_BasedEntities db,
            long trustApplicationId,
            long userId,
            TrustApplicationStep3Request request)
        {
            ...
            var existing =
                await db.tbl_TrustApplication_MinorDistribution
                    .FirstOrDefaultAsync(x =>
                        x.TrustApplicationID == trustApplicationId);
            ...
            if (request.MinorDistribution == null)
            {
                throw new BusinessException(
                    "Minor distribution instruction is required.",
                    "SAVE-TRUST-APPLICATION-STEP-3");
            }
            ...
            {
                throw new BusinessException(
                    "Minor distribution method is required.",
                    "SAVE-TRUST-APPLICATION-STEP-3");
            }
            ...
            if (existing == null)
            {
                existing =
                    new tbl_TrustApplication_MinorDistribution
                    {
                        ...
                        ReleaseAge =
                            distributionMethod == "TRUSTEE_HOLD"
                                ? request.MinorDistribution.ReleaseAge
                                : null,
        ...
        private void MapBeneficiary(tbl_TrustApplication_Beneficiary entity, TrustApplicationBeneficiaryRequest request)
        {
            ...
            // TIN exists -> unavailable reason not applicable
            if (!string.IsNullOrWhiteSpace(request.TaxIdentificationNo))
            {
                entity.TINUnavailableReason = null;
                entity.TINUnavailableExplanation = null;
                return;
