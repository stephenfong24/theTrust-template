...
namespace API_CPX.Class.Service.TrustApplication.Step1
{
    public class TrustApplicationStep1ServiceAsync
    {
        ...
        public async Task<TrustApplicationStepResult> SaveAsync(
            string merchantId,
            long userId,
            string roleCode,
            TrustApplicationStep1Request request)
        {
            if (request == null)
            {
                    ...
                    IsolationLevel.Serializable))
            {
                try
                {
                    ...
                    bool isNewApplication =
                        !request.TrustID.HasValue
                        ||
                        request.TrustID.Value <= 0;
                    ...
                    if (isNewApplication)
                    {
                        ...
                        {
                            throw new BusinessException(
                                "Only Trust Agents are allowed to create a new Trust Application.",
                                Code);
                        }
                    ...
                    await SavePersonalDetailAsync(
                        db,
                        application.RowID,
                        userId,
                        request);

                    await SaveSourceOfFundsAsync(
                        db,
                        application.RowID,
                        userId,
                        request);
                    ...
                    commonService.CompleteStepSave(
                        application,
                        1,
                        userId,
                        roleCode);
        ...
        private async Task ValidateProductAsync(Sandbox_BasedEntities db, string merchantId, string productCode)
        {
            ...
            bool exists = await db.tbl_TrustPlan.AnyAsync(x => x.ProductCode == value);
            if (!exists)
            {
