        ...
        >
          ...
          <div className="relative max-w-2xl py-12">
            ...
            <p className="mt-4 text-base leading-7 text-textSecondary">
              Secure local access for reviewing trust applications, client portfolios, payments, documents, reports, user roles and administrative controls.
            </p>
            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              {["Role access", "Local session", "Financial workflow"].map((item) => (
                <div key={item} className="rounded-lg border border-white/70 bg-white/85 p-4 shadow-soft backdrop-blur-sm">
                  <ShieldCheck className="h-5 w-5 text-brandGold" />
                  <div className="mt-3 text-sm font-semibold">{item}</div>
                </div>
              ))}
            </div>
          </div>
          <p className="relative text-xs text-textSecondary">Client review environment using local application data.</p>
        </section>
        <section className="flex items-center justify-center bg-soft p-6">
