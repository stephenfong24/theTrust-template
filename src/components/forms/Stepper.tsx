...
export function Stepper({ steps, active }: { steps: string[]; active: number }) {
  return (
    <ol
      className="grid gap-2 sm:grid-cols-2 lg:grid-cols-[repeat(var(--step-count),minmax(0,1fr))]"
    ...
    >
      {steps.map((step, index) => {
        ...
        return (
          <li
            key={step}
            className={`relative flex min-h-[68px] items-center gap-2 rounded-lg border px-3 py-2.5 transition ${itemClass}`}
          >
